...
// Workbox manifest type (injected at build time)
type WorkboxManifest = Array<{ url: string; revision: string | null }>;

// Firebase config placeholders (replaced at build time by Vite's define)
...
precacheAndRoute((self as any).__WB_MANIFEST as WorkboxManifest);
...
try {
  ...
  self.addEventListener('notificationclick', (event: NotificationEvent) => {
    ...
    const urlToOpen = event.notification.data?.url as string | undefined;
      ...
      event.waitUntil(
        self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList: readonly Client[]) => {
          const existingClient = clientList.find(
            (client: Client) => client.url === '/' && 'focus' in client
          );
          if (existingClient) {
